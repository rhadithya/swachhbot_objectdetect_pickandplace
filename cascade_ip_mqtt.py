#same as objvid2, modular --neglect this
import cv2
import numpy as np
import time 
import urllib2
...
MIN_MATCH_COUNT = 10
#frame size of 500*375
RESIZE_PARAM=500.0

#xml files of trained classifiers, here we use two files for two objects, rpi box and matchbox
cascade1 = cv2.CascadeClassifier('matchbox_250_10haar.xml')
cascade2 = cv2.CascadeClassifier('rpi_500_10haar.xml')

#setup of mqtt server to send messages to the robot
mqttc = mqtt.Client()
...
def objdetect(det,imgg1,imgg2):
    ...
    if len(good)>MIN_MATCH_COUNT:
        ...
        #using source and destination points find perspective transformation function i.e. obtain the training sample image in the  video frame
...
#############
#this section initializes the components required for ipwebcam stream
#host - ipwebcam host ip
...
img2=objresize(img2,RESIZE_PARAM)
...
while(1):
   # _,img1=cap.read()
    ...
    if a!=-1 and b!=-1:
        jpg = bytes[a:b+2]
        bytes= bytes[b+2:]
        img1 = cv2.imdecode(np.fromstring(jpg, dtype=np.uint8),cv2.CV_LOAD_IMAGE_COLOR)
        ...
        ##############################################
        objects1 =cascade1.detectMultiScale(gray, 1.1,5) #(1.1,5)
        for (x,y,w,h) in objects1:
            cv2.rectangle(img1,(x,y),(x+w,y+h),(255,0,0),2)
            roi_gray = gray[y:y+h, x:x+w]
            roi_color = img1[y:y+h, x:x+w]

        objects2 =cascade2.detectMultiScale(gray, 1.1,10)
        ...
        if counter==10:
            point=objdetect(surf,img1,img2)
            counter=0
        '''
    #print(point)
    '''
